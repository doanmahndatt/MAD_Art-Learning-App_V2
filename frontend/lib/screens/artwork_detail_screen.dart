...
class ArtworkDetailScreen extends StatefulWidget {
  ...
  @override State<ArtworkDetailScreen> createState() => _ArtworkDetailScreenState();
}
...
class _ArtworkDetailScreenState extends State<ArtworkDetailScreen> {
  ...
  @override void initState() { super.initState(); _fetchDetail(); }
  ...
  Future<void> _addComment(AppProvider app) async {
    ...
    try {
      final res = await _api.post('/comments', {'content': _commentController.text, 'artwork_id': widget.artworkId});
      if (res.statusCode == 201) { setState(() { _comments.insert(0, res.data); _commentController.clear(); }); NotificationService.showSuccess(app.t('commented')); }
    } catch (e) { NotificationService.showError('Error: $e'); }
  }
  ...
  Future<void> _editComment(String id, String old, AppProvider app) async {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      title: Text(app.t('edit_comment')), content: TextField(controller: ctrl, autofocus: true),
      actions: [
        ...
        TextButton(onPressed: () async {
          ...
          try {
            ...
            if (r.statusCode == 200) { setState(() { final i = _comments.indexWhere((x) => x['id'] == id); if (i != -1) _comments[i]['content'] = c; }); NotificationService.showSuccess(app.t('edited')); Navigator.pop(context); }
  ...
  Widget _buildImage(String url) {
    ...
    return CachedNetworkImage(imageUrl: url, fit: BoxFit.cover, width: double.infinity, height: 300,
        placeholder: (_, __) => Container(color: Colors.grey[100]), errorWidget: (_, __, ___) => const Icon(Icons.broken_image, size: 50));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // Always white for the artwork display area
      ...
      body: _loading ? Center(child: CircularProgressIndicator(color: AppColors.primary, strokeWidth: 2.5))
        ...
        // Image on white background regardless of dark mode
        ...
        Container(color: surfColor, padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(_artwork?['created_at'] != null ? _artwork!['created_at'].toString().substring(0, 10) : '',
                  style: TextStyle(fontSize: 12, color: isDark ? AppColors.darkTextLight : AppColors.textLight)),
            ])),
            IconButton(icon: Icon(_isLiked ? Icons.favorite_rounded : Icons.favorite_border_rounded, color: _isLiked ? AppColors.like : (isDark ? AppColors.darkTextLight : AppColors.textLight)), onPressed: () => _toggleLike(app)),
          ...
          Text(app.t('comments'), style: TextStyle(fontWeight: FontWeight.bold, color: textColor)),
          const SizedBox(height: 8),
          Row(children: [
          ...
          const SizedBox(height: 8),
          ListView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: _comments.length,
              itemBuilder: (_, i) { final c = _comments[i]; return CommentTile(comment: c, isOwner: c['user_id'] == auth.user?.id, onEdit: () => _editComment(c['id'], c['content'], app), onDelete: () => _deleteComment(c['id'], app)); }),
        ])),
      ])),
    );
  }
